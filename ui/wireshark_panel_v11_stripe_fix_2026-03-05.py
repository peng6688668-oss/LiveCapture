...
from PyQt6.QtCore import Qt, QThread, pyqtSignal, QProcess, QTimer, QModelIndex, QAbstractTableModel, QObject
...
class WindowsCaptureThread(QThread):
    ...
    def run(self):
        try:
            ...
            cmd = [dumpcap_path, "-i", self.interface, "-w", "-", "-P", "-q"]
...
class LiveVideoDecoder(QObject):
    ...
    def _update_0x2090_lcg_from_snapshot(self, snapshot) -> 'Optional[np.ndarray]':
        ...
        lcg_buf[idx_arr] = new_lcg

        del raw_buf, b0, b1, b2, image, new_lcg, new_data
        return lcg_buf.copy()
    ...
    def _isp_from_lcg(self, stream_id: int, lcg: 'np.ndarray',
        ...
        # ── 3. Bayer BG Demosaic ──
        bgr_16 = cv2.cvtColor(ch_16, cv2.COLOR_BayerBG2BGR)
        del ch_16
    ...
    def _reconstruct_0x2090_frame(self, stream_id: int,
        ...
        if _frame_num <= 10 or _frame_num % 20 == 0:
            try:
                with open('/tmp/0x2090_isp.log', 'a') as _f:
                    _f.write(f"frame {_frame_num} stream 0x{stream_id:x}: "
                             f"raw={len(raw_lines)} lines, gaps={gap_fills}, "
                             f"h={actual_h}, "
                             f"LCG {bh}x{bw} → BGR {bgr.shape[1]}x{bgr.shape[0]}, "
                             f"{_isp_ms:.0f}ms\n")
            except Exception:
                pass
    ...
    def _reconstruct_csi2_frame(self) -> Optional['np.ndarray']:
        ...
        try:
            ...
            if dt == 0x1E:
                ...
                return np.frombuffer(raw_data[:total], np.uint8).reshape((height, width, 3))
...
class WiresharkPanel(QWidget):
    ...
    def _init_ui(self):
        ...
        self._selected_interface = ""  # Speichert den ausgewählten Interface-Wert
        ...
        # Oberer Bereich: Horizontaler Splitter (Durchsatz | Paketliste)
    ...
    def _populate_interfaces(self):
        ...
        action = self._interface_menu.addAction("Alle (Standard)")
        action.triggered.connect(
            lambda checked: self._on_interface_selected("Alle (Standard)", ("", "wsl")))
            ...
            try:
                ...
                for iface in sorted(interfaces):
                    if iface and iface != "lo":
                        self._interface_items.append((iface, (iface, "wsl")))
                        action = self._interface_menu.addAction(iface)
                        action.triggered.connect(
                            lambda checked, n=iface: self._on_interface_selected(n, (n, "wsl")))
                if "lo" in interfaces:
                    self._interface_items.append(("lo (Loopback)", ("lo", "wsl")))
                    action = self._interface_menu.addAction("lo (Loopback)")
                    action.triggered.connect(
                        lambda checked: self._on_interface_selected("lo (Loopback)", ("lo", "wsl")))
            except Exception:
                for iface, label in [("eth0", "eth0"), ("lo", "lo (Loopback)")]:
                    self._interface_items.append((label, (iface, "wsl")))
                    action = self._interface_menu.addAction(label)
                    action.triggered.connect(
                        lambda checked, n=label, v=iface: self._on_interface_selected(n, (v, "wsl")))

        # Standardauswahl: erster Eintrag
        if self._interface_items:
            first_label, first_data = self._interface_items[0]
            self._on_interface_selected(first_label, first_data)

    def _on_interface_selected(self, label: str, user_data: tuple):
        """Wird aufgerufen, wenn ein Interface im Menü ausgewählt wird."""
        self.interface_combo.setText(label)
        self._selected_interface, self._selected_interface_type = user_data
    ...
    def _start_live_capture(self):
        ...
        if use_windows:
            ...
            self._live_capture_thread = LiveCaptureThread(interface, capture_filter, 0)
            backend = "Scapy"
        ...
        self.status_label.setText(f"Live-Capture läuft [{backend}]...{filter_info}")
    ...
    def _start_video_decode(self):
        ...
        self._video_info_label.setText("🎬 Live Video  —  Warte auf Signal...")
    ...
    def _on_video_frame_ready(self, bgr, display_index: int = 0):
        """Zeigt einen dekodierten Video-Frame an."""
        if not CV2_AVAILABLE:
            return
        try:
            # Ziel-Display bestimmen
            ...
            # Skalierung auf Display-Größe (KeepAspectRatio)
    ...
    def _on_video_info_updated(self, info: dict):
        ...
        self._video_info_label.setText(
            f"🎬 Live Video   {res}   {fps} FPS   {codec}   #{frames}"
        )
    ...
    def _rearrange_video_grid(self, count: int):
        ...
        if count == 1:
            ...
            self._video_grid_layout.addWidget(self._video_panels[0], 0, 0)
            self._video_grid_layout.addWidget(self._video_panels[1], 0, 1)
            self._video_grid_layout.addWidget(self._video_panels[2], 1, 0)
            self._video_grid_layout.addWidget(self._video_panels[3], 1, 1)
            for i in range(4):
                self._video_panels[i].setVisible(True)
